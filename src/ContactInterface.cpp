...
void ContactInterface::getVolumeAndGradient(std::vector<double> &IV, std::vector<double> &IS_BDV, int &IV_size, 
        ...
        double minSep, double periodicLength)
{
...
void ContactInterface::init(SURF_TYPE st)
{
  mesh_manager = new MeshManager();
  m_P = new Polyhedron;
  //std::ifstream fin("data/tmp.off", std::ios_base::in | std::ios_base::binary);
  //fin >> *m_P;
  Build_surface<Polyhedron::HalfedgeDS> triangles(this);
  m_P->delegate(triangles);

  m_P->setFilename("");
  m_P->setName("");
  m_P->init();
  mesh_manager->setMesh(m_P,st, m_nv, m_p);
...
void ContactInterface::fillMPos(std::vector<double> &pos, double *pos_pole)
{
...
void ContactInterface::generateMesh(std::vector<double> pos, double *pos_pole, int p, int nv)
{
  ...
  for(int iv=0;iv<m_nv;++iv)
  {
    ...
    // with north pole
    for(int i=1;i<m;++i)
      m_faces.push_back(Trig<int>(0 + iv*m_Ni, i + iv*m_Ni , i+1 + iv*m_Ni));
    m_faces.push_back(Trig<int>(0 + iv*m_Ni, m + iv*m_Ni, 1 + iv*m_Ni));
    // with south pole
    for(int i=1;i<m;++i)
      //faces.push_back(Trig<int>(N-1,N-1-m+i-1,N-1-m+i));
      m_faces.push_back(Trig<int>(m_Ni-1 + iv*m_Ni, m_Ni-1-m+i + iv*m_Ni, 
            m_Ni-1-m+i-1 + iv*m_Ni));
    //faces.push_back(Trig<int>(N-1,N-2,N-1-m));
    m_faces.push_back(Trig<int>(m_Ni-1 + iv*m_Ni, m_Ni-1-m + iv*m_Ni,
          m_Ni-2 + iv*m_Ni));

#ifdef VERBOSE
    for(std::vector< Trig<int> >::iterator 
    ...
    for(int i=1;i<n;++i)
    {
      ...
      {
        m_faces.push_back(
            Trig<int>( 1+(i-1)*m+j + iv*m_Ni, 1+i*m+j + iv*m_Ni, 
              1+i*m+j+1 + iv*m_Ni) );
        //faces.push_back(
        //    Trig<int>( 1+(i-1)*m+j,1+(i-1)*m+j+1,1+i*m+j+1) );
        m_faces.push_back(
            Trig<int>( 1+(i-1)*m+j + iv*m_Ni ,1+i*m+j+1 + iv*m_Ni,
              1+(i-1)*m+j+1 + iv*m_Ni) );
      }
      int j = m-1;
      m_faces.push_back(
          Trig<int>( 1+(i-1)*m+j + iv*m_Ni, 1+i*m+j + iv*m_Ni,
            i*m+(j+2)%m + iv*m_Ni) );
      //faces.push_back(
      //    Trig<int>( 1+(i-1)*m+j, (i-1)*m+(j+2)%m, i*m+(j+2)%m ) );
      m_faces.push_back(
          Trig<int>( 1+(i-1)*m+j + iv*m_Ni, i*m+(j+2)%m + iv*m_Ni, 
            (i-1)*m+(j+2)%m + iv*m_Ni) );
    }
